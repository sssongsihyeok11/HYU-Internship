...
Ciphertext<DCRTPoly> paterson_stock_meyer_recursive(CryptoContext<DCRTPoly> cc, int start, int end, int baby_step,
    ...
    if(end - start < baby_step){
        return honor_method(cc, start, end, coefficient, input);
    }
    int mid = start + baby_step;

    Ciphertext<DCRTPoly> left_ciphertext = paterson_stock_meyer_recursive(cc, start, mid - 1, baby_step, coefficient, input, power);
    Ciphertext<DCRTPoly> right_ciphertext = paterson_stock_meyer_recursive(cc, mid, end, baby_step, coefficient, input, power);
    return cc->EvalAdd(left_ciphertext, cc->EvalMult(right_ciphertext, power));
}
...
Ciphertext<DCRTPoly> paterson_stock_meyer_linear(CryptoContext<DCRTPoly> cc, size_t start, size_t end, size_t baby_step,
    ...
    size_t idx = 0;
    for(size_t i = start; i < end; i += baby_step){
        size_t block_end = std::min(i + baby_step - 1, end - 1);
        auto tmp = honor_method(cc, i, block_end, coefficient, input);
        if(i == start){
            res = tmp;
        }
        else{
